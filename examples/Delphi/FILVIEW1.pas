  ...
  if PrintDialog1.Execute then begin
    ...
    try
      ...
      case PrintDialog1.PrintRange of
        prAllPages :
          begin
            if Viewer1.LineCount = MaxLongInt then
              {checking this invalid line forces reading of all lines}
              Viewer1.CheckLine(MaxLongInt);
...
procedure TViewForm.Copy1Click(Sender: TObject);
begin
...
var
  S : String;
begin
  S := IntToStr(Succ(Viewer1.CaretEffectivePos.Line)) +
       ':' + IntToStr(Succ(Viewer1.CaretEffectivePos.Col));
  while Length(S) < 8 do S := S + ' ';
  Header1.Sections[0] := S;
  if Viewer1.LineCount = MaxLongInt then
    S := '?'
  else
    S := IntToStr(Viewer1.LineCount);
  Header1.Sections[1] := 'Total: ' + S;
  S := IntToStr(Succ(Viewer1.TopLine));
  while Length(S) < 5 do S := S + ' ';
  Header1.Sections[2] := 'Top: ' + S;
  Header1.Sections[3] := 'Bytes: ' + IntToStr(Viewer1.FileSize);
  Clipboard1Click(Self);
end;
...
procedure TViewForm.HexMode1Click(Sender: TObject);
begin
  HexMode1.Checked := not(HexMode1.Checked);
